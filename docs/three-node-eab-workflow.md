...
This guide is for the `10.10.10.1`, `10.10.10.2`, `10.10.10.3` lab.

## Security first

If `10.10.10.3` is tethered through a compromised Android phone, do not treat it as a validator yet.

- Run `10.10.10.1` and `10.10.10.2` as producers.
- Run `10.10.10.3` as a non-producing observer with `QCOIN_PRODUCE=false`.
- Do not store irreplaceable signing material on `10.10.10.3` until the laptop and tether path are clean.

That gives you a usable dev network without trusting the weakest link.

## Phase 1: Build once

On each qcoin machine:
...
Run the EAB service on the most trusted machine first. `10.10.10.1` is the simplest anchor node.

```bash
cd /home/jay/pudding/entitlement-achievement-blockchain
...
Use this sequence while funds are tight:
...
4. Only after the `10.10.10.3` laptop is clean, add its public key to every node's `validator_public_key_hex` array and switch it to `QCOIN_PRODUCE=true`.
